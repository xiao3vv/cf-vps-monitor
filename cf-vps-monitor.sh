...
report_metrics() {
    ...
    # 直接使用原始CPU数据中的负载数组，避免复杂的sed解析
    local cpu_load="0,0,0"
    if [[ "$cpu_raw" =~ \"load_avg\":\[([^\]]+)\] ]]; then
        local cpu_load_raw="${BASH_REMATCH[1]}"
        # 分割负载值并验证每个值
...
debug_cpu() {
    ...
    # 检查CPU使用率获取方法
    print_message "$CYAN" "1. 检查CPU使用率获取:"
    if [[ "$OS" == "FreeBSD" ]]; then
        echo "FreeBSD系统 - 使用 sysctl:"
        if command_exists sysctl; then
            echo "  kern.cp_time:"
            sysctl -n kern.cp_time 2>/dev/null || echo "    获取失败"
        else
            echo "  ✗ sysctl 命令不可用"
        fi
    else
        ...
        if command_exists top; then
            echo "  ✓ top 命令可用"
            echo "  测试 top -bn1:"
            top -bn1 | grep -i "cpu(s)" | head -1 || echo "    获取失败"
        else
            echo "  ✗ top 命令不可用"
        fi
    ...
    # 检查负载平均值获取方法
    echo
    print_message "$CYAN" "2. 检查负载平均值获取:"
    if [[ "$OS" == "FreeBSD" ]]; then
        echo "FreeBSD系统 - 使用 sysctl:"
        if command_exists sysctl; then
            echo "  vm.loadavg:"
            sysctl -n vm.loadavg 2>/dev/null || echo "    获取失败"
        else
            echo "  ✗ sysctl 命令不可用"
        fi
    else
        echo "Linux系统:"
        if [[ -f /proc/loadavg ]]; then
            echo "  ✓ /proc/loadavg 可用"
            echo "  内容: $(cat /proc/loadavg)"
        else
            echo "  ✗ /proc/loadavg 不存在"
        fi
    ...
    # 测试CPU监控函数
    echo
    print_message "$CYAN" "3. 测试CPU监控函数:"
    ...
    if command_exists jq; then
        ...
        echo "  (安装 jq 可以更好地显示JSON数据)"
        # 手动解析显示
        local usage=$(echo "$cpu_data" | sed -n 's/.*"usage_percent":\([^,}]*\).*/\1/p')
        local load_raw=$(echo "$cpu_data" | sed -n 's/.*"load_avg":\[\([^\]]*\)\].*/\1/p')
        echo "  CPU使用率: $usage%"
        echo "  负载平均值: [$load_raw]"
    fi
}
